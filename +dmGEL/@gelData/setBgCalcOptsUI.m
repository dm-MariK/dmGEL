function setBgCalcOptsUI(obj)
% # BackGround Calculator Settings
%
% * Use default BackGround Calculator function ('radiobutton')
%   Set up multipliers:
%   xMult: |<'edit'>|   yMult: |<'edit'>|
%   + Use same xMult and yMult values ('checkbox')
%   + Use default multipliers' values   ('checkbox')
%   'text' with description...
%   
% * Use custom BackGround Calculator function ('radiobutton')
...
% The default BackGround calculator function (dmGEL.bgRectImopen) is based
% upon morphological image opening algorithm. For this to work the
% rectangle sub-piece of a gel image is required. This sub-piece must be
% simultaneously "xMult"-times wider than the widest band on the gel AND
% "yMult"-times higher than the highest band. Typically, the bigest band is
% the widest and the highest one simultaneously. So select it to calculate
% and subtract the image BackGround. If the widest one and the highest one
% are different bands - just select any arbitrary area on a figure such a
% way that this area would be of the same width as the widest band and of
% the same height as the highest band. And then use this selection to
% proceed with BackGround calculation. The values of "xMult" and "yMult"
% you can adjust here. Commonly, best results are achieved with both of
% them are set to 2. Therefore this is preset default for both. But in some
% cases the other values could be more reliable. Typically but not always
% both "xMult" and "yMult" are expected to be somewhere between 1.5 and 2.5

topDsptTxt_str = {...
    'The default BackGround calculator function (dmGEL.bgRectImopen) is based upon morphological image opening', ...
    'algorithm. For this to work the rectangle sub-piece of a gel image is required. This sub-piece must simultaneously', ...
    'be "xMult"-times wider than the widest band on the gel AND "yMult"-times higher than the highest band. Typically,', ...
    'the bigest band is the widest and the highest one simultaneously. So select it to calculate and subtract the image', ...
    'BackGround. If the widest one and the highest one are different bands - just select any arbitrary area on the image', ...
    'such a way that this area would be of the same width as the widest band and of the same height as the highest', ... 
    'band. And then use this selection to proceed with BackGround calculation.', ...
    ' * The values of "xMult" and "yMult" you can adjust here. Commonly, best results are achieved with both of them', ... 
    'being set to 2. Therefore this is preset default for both. But in some cases the other values could be more reliable.', ...
    'Typically but not always both "xMult" and "yMult" are expected to be somewhere between 1.5 and 2.5.' ...
    };
...
botDsptTxt_str = {...
    ...
    'a function handle, ready to be executed. The function should return a calculated image BackGround and only it.', ...
    'The 1-st input of the function (Arg1) must be an image whose BackGround to be calculated. The 2-nd input (Arg2)', ... 
    'must be a selected area represented by polygon''s vertices pixel position.', ...
    ' * Specify additional arguments to your function in the format of a cell array in the order thay should appear in', ...
    'your function''s call, as follows: {Arg3, Arg4, Arg5} and so on. Each ArgN by itself could be of any possible format.',...
...
fig = figure('Name', 'BackGround Calculator Settings', ... 
    'WindowStyle', 'modal', ...
    'Units', 'pixels',...
    'Position', fPos, ...
...
useDfltRB = uicontrol('Parent', fig, ... 
    'Style','radiobutton', ...
    'String','Use default BackGround Calculator function', ...
...
% Set up multipliers 'text'
%sMultHdrTxt = ...
uicontrol('Parent', fig, ... 
    'Style','text', ...
    'String','Set up multipliers:', ...
    'HorizontalAlignment', 'left', ...
    'Units', 'pixels', ...
...
% 'Use same xMult and yMult' and 'Use default multipliers' 'checkbox'-es
useSameCkb = uicontrol('Parent', fig, ...
    'Style', 'checkbox', ...
    'String', ' Use same xMult and yMult values', ...
    'HorizontalAlignment', 'left', ...
    'Units', 'pixels', ...
...
useDfltCkb = uicontrol('Parent', fig, ...
    'Style', 'checkbox', ...
    'String', ' Use default multipliers'' values', ...
    'HorizontalAlignment', 'left', ...
    'Units', 'pixels', ...
...
useCustomRB = uicontrol('Parent', fig, ... 
    'Style','radiobutton', ...
    'String','Use custom BackGround Calculator function', ...
    'HorizontalAlignment', 'left', ...
    'FontWeight','bold', ...
    'Units', 'pixels', ...
    ...
    function updUI
        if get(useDfltRB, 'Value') %== true % 'Use default fcn' is selected
            ...
            if get(useDfltCkb, 'Value') %== true % 'Use default fcn's Args' is checked
                ...
                xMult = str2num(get(xEdt, 'String'));
                yMult = str2num(get(yEdt, 'String'));
                fcnArgs = {xMult, yMult};
            end
